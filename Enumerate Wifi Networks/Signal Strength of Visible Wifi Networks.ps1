# Powershell script to monitor the signal strength of all visible wifi networks using PRTG

# Largely based on https://jordanmills.wordpress.com/2013/09/26/how-to-list-ssids-in-powershell-without-parsing-command-line-output/

# added code for PRTG
# added scanning for networks
...
'@
    ...
    try {
        ...
        $wifis = $WlanClient.Interfaces | 
        ForEach-Object { 
            write-host $_.InterfaceName " is being scanned"
            $_.Scan();
        ...
        {
            write-host $wifi.ssid " => " $wifi.linkQuality

            $count=$count+1

            $wlantype="";
            if($wifi.dot11BssPhyType -eq 1) {$wlantype=" (Bluetooth)"}
            if($wifi.dot11BssPhyType -eq 4) {$wlantype=" (802.11a)"}
            if($wifi.dot11BssPhyType -eq 6) {$wlantype=" (802.11g)"}
            if($wifi.dot11BssPhyType -eq 7) {$wlantype=" (802.11n)"}
            if($wifi.dot11BssPhyType -eq 8) {$wlantype=" (802.11ac)"}

            if ($wifi.dot11BssType -ne "Infrastructure") {$wlantype=$wlantype+" "+$wifi.dot11BssType}


            $prtgresult+="    <result>`r`n"
            $prtgresult+="        <channel>"+($wifi.ssid + $wlantype)+"</channel>`r`n"
            $prtgresult+="        <customunit>%</customunit>`r`n"
            $prtgresult+="        <value>"+($wifi.linkQuality)+"</value>`r`n"
        ...
        $prtgresult+="    <result>`r`n"
        $prtgresult+="        <channel>Visible Wifi Networks</channel>`r`n"
        $prtgresult+="        <customunit>Networks</customunit>`r`n"
        $prtgresult+="        <value>"+($count)+"</value>`r`n"
        $prtgresult+="        <float>0</float>`r`n"
        $prtgresult+="        <mode>Absolute</mode>`r`n"
        $prtgresult+="    </result>`r`n"   
        $prtgresult+="<text>"+$count+" Wifi Networks Found</text>"
